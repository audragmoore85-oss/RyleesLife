  ...
  return (
    <div className="relative inline-block" ref={popoverRef}>
      <button
        ...
        className="flex items-center gap-2 bg-white/70 backdrop-blur-sm rounded-xl px-4 py-2.5 shadow-sm border border-white/50 hover:bg-white transition-all cursor-pointer dark:bg-slate-800/70 dark:border-slate-700/50 dark:hover:bg-slate-800"
      >
        ...
        <div className="text-left">
          <p className="text-xs text-slate-400 dark:text-slate-400 font-medium">Mood Calendar</p>
          <p className="text-lg font-bold text-slate-700 dark:text-slate-200 leading-none">{format(currentMonth, "MMMM")}</p>
        </div>
      </button>

      {isOpen && (
        <div className="absolute top-full left-0 mt-2 z-50 w-[280px] sm:w-[320px] bg-white/95 backdrop-blur-md rounded-2xl shadow-xl border border-white/60 p-4 animate-fade-in dark:bg-slate-800/95 dark:border-slate-700/60">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={prevMonth}
                className="w-6 h-6 rounded-md bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors cursor-pointer dark:bg-slate-700 dark:hover:bg-slate-600"
              >
          ...
          <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700 flex flex-wrap gap-1.5">
